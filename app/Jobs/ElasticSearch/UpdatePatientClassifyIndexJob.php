...
class UpdatePatientClassifyIndexJob implements ShouldQueue
{
    use Queueable;
    protected $client;
    protected $record;
    ...
    public function handle(): void
    {
        ...
        switch ($this->tableJoined) {
            case 'other_pay_source':
                ...
                foreach ($ids as $item) {
                    event(new InsertPatientClassifyIndex($item, 'patient_classify'));
                }
                break;
            case 'patient_type':
                $params = [
        ...
        event(new DeleteCache('patient_classify'));
    }
}
