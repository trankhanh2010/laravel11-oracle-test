...
class ServiceReqSttService 
{
    ...
    {
        try {
            $data = Cache::remember($this->params->serviceReqSttName . '_start_' . $this->params->start . '_limit_' . $this->params->limit . $this->params->orderByString . '_is_active_' . $this->params->isActive . '_get_all_' . $this->params->getAll, $this->params->time, function (){
                $data = $this->serviceReqSttRepository->applyJoins();
                $data = $this->serviceReqSttRepository->applyIsActiveFilter($data, $this->params->isActive);
                $count = $data->count();
                $data = $this->serviceReqSttRepository->applyOrdering($data, $this->params->orderBy, $this->params->orderByJoin);
    ...
    {
        try {
            $data = Cache::remember($this->params->serviceReqSttName . '_' . $id . '_is_active_' . $this->params->isActive, $this->params->time, function () use ($id){
                $data = $this->serviceReqSttRepository->applyJoins()
