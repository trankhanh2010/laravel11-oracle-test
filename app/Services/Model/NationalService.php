...
class NationalService 
{
    ...
    {
        try {
            $data = Cache::remember($this->params->nationalName . '_start_' . $this->params->start . '_limit_' . $this->params->limit . $this->params->orderByString . '_is_active_' . $this->params->isActive . '_get_all_' . $this->params->getAll, $this->params->time, function (){
                $data = $this->nationalRepository->applyJoins();
                $data = $this->nationalRepository->applyIsActiveFilter($data, $this->params->isActive);
                $count = $data->count();
                $data = $this->nationalRepository->applyOrdering($data, $this->params->orderBy, $this->params->orderByJoin);
