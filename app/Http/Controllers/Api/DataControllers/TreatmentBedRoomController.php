  ...
  public function treatment_bed_room_get_L_view(Request $request)
  {
      ...
      if ($keyword != null) {
          $data = $data->where(function ($query) use ($keyword) {
              $query = $query->where(DB::connection('oracle_his')->raw('lower(treatment.TDL_PATIENT_LAST_NAME)'), 'like', '%' . $keyword . '%')
                  ->orWhere(DB::connection('oracle_his')->raw('lower(treatment.TREATMENT_CODE)'), 'like', '%' . $keyword . '%');
          });
      }
