...
class TreatmentDataController extends BaseApiCacheController
{
    ...
    public function show($treatmentCode)
    {
        if ($this->checkParam()) {
            return $this->checkParam();
        }
        $data = $this->treatmentListVViewService->handleDataBaseGetWithTreatmentCode($treatmentCode);
        $paramReturn = [
