...
class MediStockController extends BaseApiCacheController
{
    public function __construct(Request $request)
    {
        ...
        if ($this->order_by != null) {
            $columns = $this->get_columns_table($this->medi_stock);
    ...
    public function medi_stock($id = null)
    {
        ...
        try {
            ...
            if ($keyword != null) {
                ...
                if ($id == null) {
                    $name = $this->medi_stock_name . '_start_' . $this->start . '_limit_' . $this->limit . $this->order_by_tring . '_is_active_' . $this->is_active . '_get_all_' . $this->get_all;
                    $param = [
                        'room:id,department_id,room_type_id',
                        'room.department:id,department_name,department_code',
                        'room.room_type:id,room_type_name,room_type_code',
                ...
                $data = get_cache_full($this->medi_stock, $param, $name, $id, $this->time, $this->start, $this->limit, $this->order_by, $this->is_active, $this->get_all);
            }
            $param_return = [
