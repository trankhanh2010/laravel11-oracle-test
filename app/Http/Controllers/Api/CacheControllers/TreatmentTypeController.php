    ...
    public function __construct(Request $request){
        ...
        if ($this->order_by != null) {
            foreach ($this->order_by as $key => $item) {
                if (!$this->treatment_type->getConnection()->getSchemaBuilder()->hasColumn($this->treatment_type->getTable(), $key)) {
    ...
    public function treatment_type($id = null)
    {
        ...
        if ($keyword != null) {
            ...
            $data = $data->where(function ($query) use ($keyword){
                $query = $query
                ->where(DB::connection('oracle_his')->raw('FUN_CONVERT_TO_UNSIGN(lower(treatment_type_code))'), 'like', '%' . $keyword . '%')
                ->orWhere(DB::connection('oracle_his')->raw('FUN_CONVERT_TO_UNSIGN(lower(treatment_type_name))'), 'like', '%' . $keyword . '%');
            });
        if ($this->is_active !== null) {
            ...
            if ($id == null) {
                $name = $this->treatment_type_name. '_start_' . $this->start . '_limit_' . $this->limit. $this->order_by_tring. '_is_active_' . $this->is_active;
                $param = [
                    'required_service'
                ];
            } else {
            ...
            $data = get_cache_full($this->treatment_type, $param, $name, $id, $this->time, $this->start, $this->limit, $this->order_by, $this->is_active);
        }
        $param_return = [
            'start' => $this->start,
            'limit' => $this->limit,
