...
class DiimTypeController extends BaseApiCacheController
{
    ...
    public function diim_type($id = null)
    {
        ...
        if ($this->is_active !== null) {
            ...
            if ($id == null) {
                $name = $this->diim_type_name. '_start_' . $this->start . '_limit_' . $this->limit. $this->order_by_tring;
                $param = [
                ];
            } else {
                if (!is_numeric($id)) {
            ...
            $data = get_cache_full($this->diim_type, $param, $name, $id, $this->time, $this->start, $this->limit, $this->order_by);
        }
        $param_return = [
            'start' => $this->start,
            'limit' => $this->limit,
