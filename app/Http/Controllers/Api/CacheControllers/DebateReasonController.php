...
class DebateReasonController extends BaseApiCacheController
{
    public function __construct(Request $request)
    {
        ...
        if ($this->order_by != null) {
            $columns = $this->get_columns_table($this->debate_reason);
    ...
    public function debate_reason($id = null)
    {
        ...
        try {
            ...
            if ($keyword != null) {
                $data = $this->debate_reason
                    ->select(
                        'his_debate_reason.*',
                    );
                $data = $data->where(function ($query) use ($keyword) {
                    $query = $query
                        ->where(DB::connection('oracle_his')->raw('his_debate_reason.debate_reason_code'), 'like', $keyword . '%');
                });
                if ($this->is_active !== null) {
                    $data = $data->where(function ($query) {
                        $query = $query->where(DB::connection('oracle_his')->raw('his_debate_reason.is_active'), $this->is_active);
                    });
                }
                $count = $data->count();
                if ($this->order_by != null) {
                    foreach ($this->order_by as $key => $item) {
                        $data->orderBy('his_debate_reason.' . $key, $item);
                    }
                }
                if($this->get_all){
                    $data = $data
                ...
                if ($id == null) {
                    $data = Cache::remember($this->debate_reason_name . '_start_' . $this->start . '_limit_' . $this->limit . $this->order_by_tring . '_is_active_' . $this->is_active. '_get_all_' . $this->get_all, $this->time, function () {
                        $data = $this->debate_reason
                        ->select(
                            'his_debate_reason.*',
                        );
                    ...
                    $data = Cache::remember($this->debate_reason_name . '_' . $id . '_is_active_' . $this->is_active, $this->time, function () use ($id) {
                        $data = $this->debate_reason
                        ->select(
                            'his_debate_reason.*',
                        )
