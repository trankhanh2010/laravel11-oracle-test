...
class ExeServiceModuleController extends BaseApiCacheController
{
    public function __construct(Request $request){
        ...
        if ($this->order_by != null) {
            foreach ($this->order_by as $key => $item) {
                if (!$this->exe_service_module->getConnection()->getSchemaBuilder()->hasColumn($this->exe_service_module->getTable(), $key)) {
    ...
    public function exe_service_module($id = null)
    {
        ...
        if ($this->is_active !== null) {
            ...
            if ($id == null) {
                $name = $this->exe_service_module_name. '_start_' . $this->start . '_limit_' . $this->limit. $this->order_by_tring;
                $param = [
                ];
            } else {
                if (!is_numeric($id)) {
            ...
            $data = get_cache_full($this->exe_service_module, $param, $name, $id, $this->time, $this->start, $this->limit, $this->order_by);
        }
        $param_return = [
            'start' => $this->start,
            'limit' => $this->limit,
