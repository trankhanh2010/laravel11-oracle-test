    ...
    public function __construct(Request $request)
    {
        ...
        if ($this->order_by != null) {
            foreach ($this->order_by as $key => $item) {
                if (!$this->priority_type->getConnection()->getSchemaBuilder()->hasColumn($this->priority_type->getTable(), $key)) {
    ...
    public function priority_type($id = null)
    {
        ...
        if ($keyword != null) {
            ...
            $data = $data->where(function ($query) use ($keyword){
                $query = $query
                ->where(DB::connection('oracle_his')->raw('FUN_CONVERT_TO_UNSIGN(lower(priority_type_code))'), 'like', '%' . $keyword . '%')
                ->orWhere(DB::connection('oracle_his')->raw('FUN_CONVERT_TO_UNSIGN(lower(priority_type_name))'), 'like', '%' . $keyword . '%');
            });
        if ($this->is_active !== null) {
            ...
            if ($id == null) {
                $name = $this->priority_type_name . '_start_' . $this->start . '_limit_' . $this->limit . $this->order_by_tring. '_is_active_' . $this->is_active;
                $param = [
                ];
            } else {
                if (!is_numeric($id)) {
