...
class AccidentLocationController extends BaseApiCacheController
{
    ...
    public function accident_location($id = null)
    {
        ...
        try {
            ...
            if ($keyword != null) {
                $data = $this->accident_location
                    ->select(
                        'his_accident_location.*',
                    );
                $data = $data->where(function ($query) use ($keyword) {
                    $query = $query
                        ->where(DB::connection('oracle_his')->raw('his_accident_location.accident_location_code'), 'like', $keyword . '%');
                });
                if ($this->is_active !== null) {
                    $data = $data->where(function ($query) {
                        $query = $query->where(DB::connection('oracle_his')->raw('his_accident_location.is_active'), $this->is_active);
                    });
                }
                $count = $data->count();
                if ($this->order_by != null) {
                    foreach ($this->order_by as $key => $item) {
                        $data->orderBy('his_accident_location.' . $key, $item);
                    }
                }
                $data = $data
                    ->skip($this->start)
                ...
                if ($id == null) {
                    $data = Cache::remember($this->accident_location_name . '_start_' . $this->start . '_limit_' . $this->limit . $this->order_by_tring . '_is_active_' . $this->is_active, $this->time, function () {
                        $data = $this->accident_location
                        ->select(
                            'his_accident_location.*',
                        );
