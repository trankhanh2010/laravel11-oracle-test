    ...
    public function pttt_group($id = null)
    {
        ...
        if ($keyword != null) {
            ...
            $data = $data->where(function ($query) use ($keyword){
                $query = $query
                ->where(DB::connection('oracle_his')->raw('pttt_group_code'), 'like', $keyword . '%')
                ->orWhere(DB::connection('oracle_his')->raw('pttt_group_name'), 'like', $keyword . '%');
            });
        if ($this->is_active !== null) {
            ...
            if ($id == null) {
                $name = $this->pttt_group_name. '_start_' . $this->start . '_limit_' . $this->limit. $this->order_by_tring. '_is_active_' . $this->is_active. '_get_all_' . $this->get_all;
                $param = [
                    'serv_segrs:id,service_id,service_group_id',
                    'serv_segrs.service:id,service_name,service_type_id',
                    'serv_segrs.service.service_type:id,service_type_name,service_type_code',
            ...
            $data = get_cache_full($this->pttt_group, $param, $name, $id, $this->time, $this->start, $this->limit, $this->order_by, $this->is_active, $this->get_all);
        }
        $param_return = [
