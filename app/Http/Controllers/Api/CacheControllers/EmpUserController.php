    ...
    public function emp_user($id = null)
    {
        ...
        if ($keyword != null) {
            ...
            $data = $data->where(function ($query) use ($keyword){
                $query = $query
                ->where(DB::connection('oracle_his')->raw('lower(loginname)'), 'like', '%' . $keyword . '%')
                ->orWhere(DB::connection('oracle_his')->raw('lower(tdl_username)'), 'like', '%' . $keyword . '%');
            });
        if ($this->is_active !== null) {
            ...
            if ($id == null) {
                $name = $this->emp_user_name . '_start_' . $this->start . '_limit_' . $this->limit . $this->order_by_tring;
                $param = [
                ];
            } else {
                if (!is_numeric($id)) {
