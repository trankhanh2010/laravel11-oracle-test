...
class ExecuteRoleController extends BaseApiCacheController
{
    public function __construct(Request $request)
    {
        ...
        if ($this->order_by != null) {
            $columns = $this->get_columns_table($this->execute_role);
    ...
    public function execute_role($id = null)
    {
        ...
        if ($keyword != null) {
            ...
            $data = $data->where(function ($query) use ($keyword){
                $query = $query
                ->where(DB::connection('oracle_his')->raw('execute_role_code'), 'like', $keyword . '%')
                ->orWhere(DB::connection('oracle_his')->raw('execute_role_name'), 'like', $keyword . '%');
            });
        if ($this->is_active !== null) {
            ...
            if ($id == null) {
                $name = $this->execute_role_name . '_start_' . $this->start . '_limit_' . $this->limit. $this->order_by_tring. '_is_active_' . $this->is_active. '_get_all_' . $this->get_all;
                $param = [];
            } else {
                if (!is_numeric($id)) {
                    return return_id_error($id);
                }
                $check_id = $this->check_id($id, $this->execute_role, $this->execute_role_name);
                if($check_id){
                    return $check_id; 
                }
            ...
            $data = get_cache_full_select($this->execute_role, $param, $select, $name, $id, $this->time, $this->start, $this->limit, $this->order_by, $this->is_active, $this->get_all);
        }
        $param_return = [
            $this->get_all_name => $this->get_all,
            $this->start_name => ($this->get_all || !is_null($id)) ? null : $this->start,
            $this->limit_name => ($this->get_all || !is_null($id)) ? null : $this->limit,
            $this->count_name => $count ?? ($data['count'] ?? null) ?? null,
