...
class ScreenSaverModuleLinkController extends BaseApiCacheController
{
    public function __construct(Request $request)
    {
        ...
        if ($this->order_by != null) {
            $columns = $this->get_columns_table($this->module);
            $this->order_by = $this->check_order_by($this->order_by, $columns, $this->order_by_join ?? []);
            $this->order_by_tring = arrayToCustomString($this->order_by);
        }
    }

    public function screen_saver_module_link($id = null)
    {
        ...
        try {
            ...
            if ($keyword != null) {
                ...
                $data = Cache::remember('screen_saver_module_link' . '_start_' . $this->start . '_limit_' . $this->limit . $this->order_by_tring . '_is_active_' . $this->is_active . '_get_all_' . $this->get_all, $this->time, function () {
                    $data = $this->module::select('acs_module.*')
                        ->join('ACS_MODULE_GROUP', 'ACS_MODULE.module_group_id', '=', 'ACS_MODULE_GROUP.id')
                        ->where('ACS_MODULE_GROUP.module_group_code', 'MHC');
                    if ($this->is_active !== null) {
