...
class IcdCmController extends BaseApiCacheController
{
    public function __construct(Request $request){
        ...
        if ($this->order_by != null) {
            $columns = $this->get_columns_table($this->icd_cm);
    ...
    public function icd_cm($id = null)
    {
        ...
        if ($keyword != null) {
            ...
            $data = $data->where(function ($query) use ($keyword){
                $query = $query
                ->where(DB::connection('oracle_his')->raw('icd_cm_code'), 'like', $keyword . '%')
                ->orWhere(DB::connection('oracle_his')->raw('icd_cm_name'), 'like', $keyword . '%');
            });
        if ($this->is_active !== null) {
            ...
            if ($id == null) {
                $name = $this->icd_cm_name. '_start_' . $this->start . '_limit_' . $this->limit. $this->order_by_tring. '_is_active_' . $this->is_active. '_get_all_' . $this->get_all;
                $param = [
                ];
            } else {
                if (!is_numeric($id)) {
            ...
            $data = get_cache_full($this->icd_cm, $param, $name, $id, $this->time, $this->start, $this->limit, $this->order_by, $this->is_active, $this->get_all);
        }
        $param_return = [
