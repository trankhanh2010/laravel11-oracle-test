        ...
        try {
            ...
            if (($keyword != null || $this->elastic_search_type != null) && !$this->cache) {
                if ($this->elastic_search_type != null) {
                    $data = $this->elastic_search_service->handleElasticSearchSearch($this->accident_body_part_name)['data'];
                    $count = $this->elastic_search_service->handleElasticSearchSearch($this->accident_body_part_name)['count'];
                } else {
                    $data = $this->accident_body_part_service->handleDataBaseSearch($keyword, $this->is_active, $this->order_by, $this->order_by_join, $this->get_all, $this->start, $this->limit)['data'];
                    $count = $this->accident_body_part_service->handleDataBaseSearch($keyword, $this->is_active, $this->order_by, $this->order_by_join, $this->get_all, $this->start, $this->limit)['count'];
                }
            } else {
                if ($id == null) {
                    if($this->elastic){
