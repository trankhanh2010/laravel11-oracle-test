...
class UpdateModuleRequest extends FormRequest
{
    ...
    public function rules()
    {
        ...
        if(!is_numeric($this->module)){
            throw new HttpResponseException(returnIdError($this->module));
        }
        return [
            'module_link' =>        [
                                                    'required',
                                                    'string',
                                                    'max:200',
                                                    Rule::unique('App\Models\ACS\Module')->ignore($this->module),
                                                ],
            ...
            'parent_id'  => [
                                ...
                                ->where(function ($query) {
                                    $query = $query
                                    ->where(DB::connection('oracle_acs')->raw("is_active"), 1)
                                    ->where(DB::connection('oracle_acs')->raw("application_id"), $this->application_id);
                                }),
                            ],   
    ...
    public function messages()
    {
        return [
            ...
            'parent_id.exists'      => config('keywords')['module']['parent_id'].config('keywords')['error']['exists']. ' Hoặc'. config('keywords')['error']['not_in_application_id'] ,
