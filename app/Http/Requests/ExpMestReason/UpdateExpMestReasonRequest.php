...
class UpdateExpMestReasonRequest extends FormRequest
{
    /**
    ...
    public function rules()
    {
        ...
        if(!is_numeric($this->exp_mest_reason)){
            throw new HttpResponseException(returnIdError($this->exp_mest_reason));
        }
        return [
            'exp_mest_reason_code' =>        [
                                                    'required',
                                                    'string',
                                                    'max:2',
                                                    Rule::unique('App\Models\HIS\ExpMestReason')->ignore($this->exp_mest_reason),
                                                ],
