...
class CreateIcdRequest extends FormRequest
{
    /**
    ...
    {
        return [
            'icd_code' =>           'required|string|max:10|unique:App\Models\HIS\Icd,icd_code',
        ...
        $validator->after(function ($validator) {
            if ($this->has('attach_icd_codes_list') && ($this->attach_icd_codes_list[0] != null)) {
                foreach ($this->attach_icd_codes_list as $id) {
                    if (!is_string($id) || !\App\Models\HIS\Icd::where('icd_code', $id)->where('is_active', 1)->first()) {
