...
class UpdateFuexTypeRequest extends FormRequest
{
    /**
    ...
    public function rules()
    {
        ...
        if(!is_numeric($this->fuex_type)){
            throw new HttpResponseException(returnIdError($this->fuex_type));
        }
        return [
            'fuex_type_code' =>        [
                                                    'required',
                                                    'string',
                                                    'max:10',
                                                    Rule::unique('App\Models\HIS\FuexType')->ignore($this->fuex_type),
                                                ],
