...
class UpdateMedicalContractRequest extends FormRequest
{
    /**
    ...
    public function rules()
    {
        ...
        if(!is_numeric($this->medical_contract)){
            throw new HttpResponseException(returnIdError($this->medical_contract));
        }
        return [
            'medical_contract_code' =>        [
                                                    'required',
                                                    'string',
                                                    'max:50',
                                                    Rule::unique('App\Models\HIS\MedicalContract')->ignore($this->medical_contract),
                                                ],
