...
class UpdateAgeTypeRequest extends FormRequest
{
    /**
    ...
    public function rules()
    {
        ...
        if(!is_numeric($this->age_type)){
            throw new HttpResponseException(returnIdError($this->age_type));
        }
        return [
            'age_type_code' =>        [
                                                    'required',
                                                    'string',
                                                    'max:10',
                                                    Rule::unique('App\Models\HIS\AgeType')->ignore($this->age_type),
                                                ],
