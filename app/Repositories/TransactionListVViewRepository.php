...
class TransactionListVViewRepository
{
    ...
    public function applyJoins()
    {
        return $this->transactionListVView
            ->select(
                'v_his_transaction_list.*'
            );
    }
    public function applyKeywordFilter($query, $keyword)
    {
        return $query->where(function ($query) use ($keyword) {
            $query->where(DB::connection('oracle_his')->raw('v_his_transaction_list.loginname'), 'like', $keyword . '%');
        });
    }
    public function applyIsActiveFilter($query, $isActive)
    {
        if ($isActive !== null) {
            $query->where(DB::connection('oracle_his')->raw('v_his_transaction_list.is_active'), $isActive);
        }
        return $query;
    }
    ...
    {
        if ($isDelete !== null) {
            $query->where(DB::connection('oracle_his')->raw('v_his_transaction_list.is_delete'), $isDelete);
        }
        return $query;
    }
    ...
    {
        if ($ids !== null) {
            $query->whereIn(DB::connection('oracle_his')->raw('v_his_transaction_list.transaction_type_id'), $ids);
        }
        return $query;
    }
    ...
    {
        if ($param !== null) {
            $query->where(DB::connection('oracle_his')->raw('v_his_transaction_list.transaction_code'), $param);
        }
        return $query;
    }
    ...
    {
        if ($param !== null) {
            $query->where(DB::connection('oracle_his')->raw('v_his_transaction_list.tdl_treatment_code'), $param);
        }
        return $query;
    }
        ...
        if ($orderBy != null) {
            foreach ($orderBy as $key => $item) {
                if (in_array($key, $orderByJoin)) {
                    ...
                    $query->orderBy('v_his_transaction_list.' . $key, $item);
                }
            }
        }
    ...
    public function getDataFromDbToElastic($batchSize = 5000, $id = null)
    {
        ...
        if ($id != null) {
            $data = $this->applyJoins()->where('v_his_transaction_list.id', '=', $id)->first();
            if ($data) {
            ...
            $minId = $this->applyJoins()->min('v_his_transaction_list.id');
            $maxId = $this->applyJoins()->max('v_his_transaction_list.id');
