...
class DebateReasonRepository
{
    ...
    public function getDataFromDbToElastic($batchSize = 5000, $id = null)
    {
        ...
        if ($id != null) {
            $data = $this->applyJoins()->where('his_debate_reason.id', '=', $id)->first();
            if ($data) {
            ...
            $minId = $this->applyJoins()->min('his_debate_reason.id');
            $maxId = $this->applyJoins()->max('his_debate_reason.id');
            ...
            for ($i = 0; $i < $numJobs; $i++) {
                ...
                ProcessElasticIndexingJob::dispatch('debate_reason', 'his_debate_reason', $startId, $endId, $batchSize);
            }
        }
    }
}