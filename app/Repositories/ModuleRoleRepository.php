...
class ModuleRoleRepository
{
    ...
    public function getDataFromDbToElastic($batchSize = 5000, $id = null)
    {
        ...
        if ($id != null) {
            $data = $this->applyJoins()->where('acs_module_role.id', '=', $id)->first();
            if ($data) {
            ...
            $minId = $this->applyJoins()->min('acs_module_role.id');
            $maxId = $this->applyJoins()->max('acs_module_role.id');
            ...
            for ($i = 0; $i < $numJobs; $i++) {
                ...
                ProcessElasticIndexingJob::dispatch('module_role', 'acs_module_role', $startId, $endId, $batchSize);
            }
        }
    }
}
