...
class InteractionReasonRepository
{
    ...
    public function getDataFromDbToElastic($batchSize = 5000, $id = null)
    {
        ...
        if ($id != null) {
            $data = $this->applyJoins()->where('his_interaction_reason.id', '=', $id)->first();
            if ($data) {
            ...
            $minId = $this->applyJoins()->min('his_interaction_reason.id');
            $maxId = $this->applyJoins()->max('his_interaction_reason.id');
            ...
            for ($i = 0; $i < $numJobs; $i++) {
                ...
                ProcessElasticIndexingJob::dispatch('interaction_reason', 'his_interaction_reason', $startId, $endId, $batchSize);
            }
        }
    }
}