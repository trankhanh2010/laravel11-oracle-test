...
class MediStockMetyRepository
{
    ...
    public function getDataFromDbToElastic($batchSize = 5000, $id = null)
    {
        ...
        if ($id != null) {
            $data = $this->applyJoins()->where('his_medi_stock_mety.id', '=', $id)->first();
            if ($data) {
            ...
            $minId = $this->applyJoins()->min('his_medi_stock_mety.id');
            $maxId = $this->applyJoins()->max('his_medi_stock_mety.id');
            ...
            for ($i = 0; $i < $numJobs; $i++) {
                ...
                ProcessElasticIndexingJob::dispatch('medi_stock_mety', 'his_medi_stock_mety', $startId, $endId, $batchSize);
            }
        }
    }
}
