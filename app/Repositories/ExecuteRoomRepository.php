...
class ExecuteRoomRepository
{
    ...
    public function getDataFromDbToElastic($batchSize = 5000, $id = null)
    {
        ...
        if ($id != null) {
            $data = $this->applyJoins()->where('his_execute_room.id', '=', $id)->first();
            if ($data) {
            ...
            $minId = $this->applyJoins()->min('his_execute_room.id');
            $maxId = $this->applyJoins()->max('his_execute_room.id');
            ...
            for ($i = 0; $i < $numJobs; $i++) {
                ...
                ProcessElasticIndexingJob::dispatch('execute_room', 'his_execute_room', $startId, $endId, $batchSize);
            }
        }
    }
}