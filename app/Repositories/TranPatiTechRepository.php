...
class TranPatiTechRepository
{
    ...
    public function getDataFromDbToElastic($batchSize = 5000, $id = null)
    {
        ...
        if ($id != null) {
            $data = $this->applyJoins()->where('his_tran_pati_tech.id', '=', $id)->first();
            if ($data) {
            ...
            $minId = $this->applyJoins()->min('his_tran_pati_tech.id');
            $maxId = $this->applyJoins()->max('his_tran_pati_tech.id');
            ...
            for ($i = 0; $i < $numJobs; $i++) {
                ...
                ProcessElasticIndexingJob::dispatch('tran_pati_tech', 'his_tran_pati_tech', $startId, $endId, $batchSize);
            }
        }
    }
}