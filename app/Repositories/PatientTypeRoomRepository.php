...
class PatientTypeRoomRepository
{
    ...
    public function getDataFromDbToElastic($batchSize = 5000, $id = null)
    {
        ...
        if ($id != null) {
            $data = $this->applyJoins()->where('his_patient_type_room.id', '=', $id)->first();
            if ($data) {
            ...
            $minId = $this->applyJoins()->min('his_patient_type_room.id');
            $maxId = $this->applyJoins()->max('his_patient_type_room.id');
            ...
            for ($i = 0; $i < $numJobs; $i++) {
                ...
                ProcessElasticIndexingJob::dispatch('patient_type_room', 'his_patient_type_room', $startId, $endId, $batchSize);
            }
        }
    }
}
