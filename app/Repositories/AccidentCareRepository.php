...
class AccidentCareRepository
{
    ...
    public function getDataFromDbToElastic($batchSize = 5000, $id = null)
    {
        ...
        if ($id != null) {
            $data = $this->applyJoins()->where('his_accident_care.id', '=', $id)->first();
            if ($data) {
            ...
            $minId = $this->applyJoins()->min('his_accident_care.id');
            $maxId = $this->applyJoins()->max('his_accident_care.id');
            ...
            for ($i = 0; $i < $numJobs; $i++) {
                ...
                ProcessElasticIndexingJob::dispatch('accident_care', 'his_accident_care', $startId, $endId, $batchSize);
            }
        }
    }
}
