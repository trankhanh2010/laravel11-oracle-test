...
class AgeTypeRepository
{
    ...
    public function getDataFromDbToElastic($batchSize = 5000, $id = null)
    {
        ...
        if ($id != null) {
            $data = $this->applyJoins()->where('his_age_type.id', '=', $id)->first();
            if ($data) {
            ...
            $minId = $this->applyJoins()->min('his_age_type.id');
            $maxId = $this->applyJoins()->max('his_age_type.id');
            ...
            for ($i = 0; $i < $numJobs; $i++) {
                ...
                ProcessElasticIndexingJob::dispatch('age_type', 'his_age_type', $startId, $endId, $batchSize);
            }
        }
    }
}
